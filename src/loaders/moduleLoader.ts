...
// 添加新的loader
function injectLoader(loaders: any[], index: number, options: LoaderOptions) {
  ...
  loaders.splice(index + 1, 0, ...injected)
}
...
function checkAndSetLoader(loaderContext: LoaderContext, options: LoaderOptions) {
  ...
  if (preProcessorLoaderIndex !== -1) {
    injectLoader(loaders, preProcessorLoaderIndex, options)
  }
}
