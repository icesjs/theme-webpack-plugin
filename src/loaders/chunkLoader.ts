...
export const pitch: PluginLoader['pitch'] = function () {
  ...
  if (loaders.length < 2) {
    ...
    // 首次由主题模块请求进入
    // 模块默认导出的是通过file-loader发布资源后的资源路径，或者被抽取的style内容
    // 变量的抽取将由vars-loader处理
    const resource = stringifyRequest(this, __filename + '!' + this.resource)
    const locals = style ? 'style' : 'cssPath'

    const imports = esModule
      ? `import ${locals} from ${resource}`
      : `const ${locals} = require(${resource})`
    const exports = esModule ? `export default ${locals}` : `module.exports = ${locals}`
    ...
    // 转发资源请求时进入
    // 检查并添加新的loader
    checkAndSetLoader(this, pluginOptions, query)
